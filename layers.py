...
class EventEncoder(nn.Module):
    def __init__(self,
        ...
        self.cat_embeddings = nn.ModuleList([
            nn.Embedding(self.vocab_sizes[i], self.hidden_dim) for i in range(len(self.vocab_sizes))
        ])

        self.batch_norm = nn.BatchNorm1d(len(self.cont_feature_indexes))
        self.cont_embeddings = nn.Linear(len(self.cont_feature_indexes), self.hidden_dim)

        self.global_linear = nn.Linear((len(self.vocab_sizes) + 1) * self.hidden_dim, self.output_dim)
    ...
    def forward(self, input_features: torch.Tensor) -> torch.Tensor:
        ...
        cat_embeddings = torch.cat([
            self.cat_embeddings[i](input_features[:, :, self.cat_feature_indexes[i]]) for i in
            range(len(self.vocab_sizes))
        ], dim=2)
