    ...
    def __get_sequences(self):
        ...
        for i in tqdm(range(agg_col.shape[0])):
            if agg_col[i] != curr_val:
                self.cat_sequences.append(self.df[self.cat_features].iloc[curr_ind:i].to_numpy().astype("int32"))
                self.cont_sequences.append(self.df[self.cont_features].iloc[curr_ind:i].to_numpy().astype("float32"))
